...
alerts_db_url = f"postgresql://{alerts_user}:{alerts_pwd}@{alerts_host}:{alerts_port}/{alerts_db}"
...
scores_db_url = f"postgresql://{scores_user}:{scores_pwd}@{scores_host}:{scores_port}/{scores_db}"
...
def connect_scores() -> Engine:
    engine: Engine = create_engine(scores_db_url, echo=False, connect_args={'options': '-csearch_path={}'.format(scores_schema)})
    return engine
