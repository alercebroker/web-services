...
class Mastercat(BaseModel):
    ...
    def to_dict(self):
        return {"id": self.id, "ra": self.ra, "dec": self.dec, "cat": self.cat}


def find_objects_by_ipix(
    ipix: List[int], db_pool: ConnectionPool, catalog: str = "all"
) -> List[Mastercat]:
    ...
    with db_pool.connection() as conn:
        with conn.cursor() as cur:
            if catalog == "all":
                query = (
                    "SELECT id, ra, dec, cat FROM mastercat WHERE ipix IN ({})".format(
                        ",".join(["%s"] * len(ipix))
                    )
                )
                cur.execute(query, ipix)
            else:
                query = "SELECT id, ra, dec, cat FROM mastercat WHERE ipix IN ({}) AND cat = %s".format(
                    ",".join(["%s"] * len(ipix))
                )
