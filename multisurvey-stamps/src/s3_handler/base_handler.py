...
class BaseS3Handler():
    ...
    def __init__(self, bucket_name: str, bucket_region: str, valid_stamps_type: list[str], compressed):
    ...
    def _get_stamp(self, avro_name : str, avro_data : dict, stamp_type: str, file_format: str):
    ...
    def get_stamp(self, oid : str, measurement_id: str, stamp_type: str, file_format: str):
        avro_name = self._get_avro_name(oid, measurement_id)
        avro_data = self._get_file_from_s3(avro_name)

        file_name, file, mime = self._get_stamp(avro_name, avro_data, stamp_type, file_format)
