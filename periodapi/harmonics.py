...
def compute_chi_squared(lightcurve_with_period):
    ...
    for band in ['g', 'r']:
        ...
        brightness = band_observations['brightness'].astype(np.float64).values.flatten()
        error = band_observations['e_brightness'].astype(np.float64).values.flatten() + 10 ** -2

        best_freq = 1 / period

        omega = [np.array([[1.] * len(time)])]
        timefreq = (2.0 * np.pi * best_freq
                    * np.arange(1, n_harmonics + 1)).reshape(1, -1).T * time
        ...
        chi = np.sum((fitted_magnitude - brightness) ** 2 / (error + error_tol) ** 2)
