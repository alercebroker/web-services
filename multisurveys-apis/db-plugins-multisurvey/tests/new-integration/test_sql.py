...
class ObjectModelTest(unittest.TestCase):
    ...
    def test_filter_objects(self):
        ...
        with self.db.session() as session:
            ...
            query1 = select(Object).where(Object.stellar)
            stellar_objects = list(session.execute(query1).scalars())
            self.assertEqual(
                len(stellar_objects), 2
            )  # Debería encontrar 2 objetos

            # Filtrar por tid=1 y stellar=True
            query2 = select(Object).where(Object.tid == 1, Object.stellar)
...
class ZtfObjectModelTest(unittest.TestCase):
    ...
    def test_create_multiple_objects(self):
        ...
        with self.db.session() as session:
            ...
            # Verificar que el número de objetos guardados coincide con los creados
            self.assertEqual(
                len(saved_objects), len(ZTF_OBJECT_DATA["filter"])
            )
    ...
    def test_filter_objects(self):
        ...
        with self.db.session() as session:
            ...
            query3 = select(ZtfObject).where(
                ZtfObject.g_r_max_corr.between(1.1, 1.5)
            )
            mid_corr_objects = list(session.execute(query3).scalars())
            self.assertEqual(
                len(mid_corr_objects), 3
            )  # Debería encontrar 3 objetos
...
class ZtfDetectionModelTest(unittest.TestCase):
    ...
    def test_create_multiple_detections(self):
        ...
        detections = [
            ZtfDetection(**data) for data in ZTF_DETECTION_DATA["filter"]
        ]
        ...
        with self.db.session() as session:
            ...
            self.assertEqual(
                len(saved_detections), len(ZTF_DETECTION_DATA["filter"])
            )
    ...
    def test_filter_detections(self):
        ...
        with self.db.session() as session:
            ...
            query2 = select(ZtfDetection).where(ZtfDetection.isdiffpos)
            ...
            query3 = select(ZtfDetection).where(
                ZtfDetection.rb.between(0.8, 0.9)
            )
            ...
            query4 = select(ZtfDetection).where(
                ...
                ZtfDetection.isdiffpos == False,  # noqa: E712
            )
            combined_filter_detections = list(
                session.execute(query4).scalars()
            )
            self.assertEqual(
            ...
            self.assertEqual(
                sorted(
                    [
                        (det.oid, det.measurement_id)
                        for det in bright_detections
                    ]
                ),
                sorted(
                    [
            ...
            self.assertEqual(
                sorted(
                    [
                        (det.oid, det.measurement_id)
                        for det in positive_diff_detections
                    ]
                ),
...
class ForcedPhotometryModelTest(unittest.TestCase):
    ...
    def test_filter_forced_photometry(self):
        ...
        measurements = [
            ForcedPhotometry(**data)
            for data in FORCED_PHOTOMETRY_DATA["filter"]
        ]
        with self.db.session() as session:
            for m in measurements:
                session.add(m)
        ...
        with self.db.session() as session:
            ...
            query1 = select(ForcedPhotometry).where(
            ...
            combined_filter_measurements = list(
                session.execute(query4).scalars()
            )
            self.assertEqual(
            ...
            self.assertEqual(
                sorted(
                    [(m.oid, m.measurement_id) for m in later_measurements]
                ),
                sorted(
                    [
                        (12345680, 987654322),
            ...
            self.assertEqual(
                sorted(
                    [(m.oid, m.measurement_id) for m in band1_measurements]
                ),
...
class ZtfForcedPhotometryModelTest(unittest.TestCase):
    ...
    def test_create_multiple_ztf_forced_photometry(self):
        ...
        measurements = [
            ZtfForcedPhotometry(**data) for data in photometry_data
        ]
    ...
    def test_filter_ztf_forced_photometry(self):
        ...
        # Crear y agregar múltiples mediciones a la base de datos
        measurements = [
            ZtfForcedPhotometry(**data)
            for data in ZTF_FORCED_PHOTOMETRY_DATA["filter"]
        ]
        with self.db.session() as session:
            for m in measurements:
                session.add(m)
        ...
        with self.db.session() as session:
            ...
            query1 = select(ZtfForcedPhotometry).where(
            ...
            # Filtrar por rband = 1
            query2 = select(ZtfForcedPhotometry).where(
                ZtfForcedPhotometry.rband == 1
            )
            band1_measurements = list(session.execute(query2).scalars())
            self.assertEqual(
                len(band1_measurements), 2
            ...
            query6 = select(ZtfForcedPhotometry).where(
                ZtfForcedPhotometry.mag < 19.0, ZtfForcedPhotometry.corrected
            )
            combined_filter_measurements = list(
                session.execute(query6).scalars()
            )
            self.assertEqual(
                len(combined_filter_measurements), 1
            )  # Debería encontrar 1 medición
            ...
            self.assertEqual(
                sorted(
                    [(m.oid, m.measurement_id) for m in bright_measurements]
                ),
                sorted(
                    [
                        (12345680, 987654321),
            ...
            self.assertEqual(
                sorted(
                    [(m.oid, m.measurement_id) for m in corrected_measurements]
                ),
...
class NonDetectionModelTest(unittest.TestCase):
    ...
    def test_create_multiple_non_detections(self):
        ...
        with self.db.session() as session:
            ...
            self.assertEqual(
                len(saved_non_detections), len(non_detection_data)
            )
    ...
    def test_filter_non_detections(self):
        ...
        non_detections = [
            NonDetection(**data) for data in NON_DETECTION_DATA["filter"]
        ]
        with self.db.session() as session:
            for nd in non_detections:
                session.add(nd)
        ...
        with self.db.session() as session:
            ...
            query3 = select(NonDetection).where(
                NonDetection.mjd.between(60035, 60041)
            )
            ...
            combined_filter_non_detections = list(
                session.execute(query5).scalars()
            )
            self.assertEqual(
