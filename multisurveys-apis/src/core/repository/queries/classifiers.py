...
from db_plugins.db.sql.models import classifier_ms, Taxonomy_ms, Detection
...
def get_classifier_by_name(
    ...
    with session_factory() as session:
        stmt = (
            ...
            .join(Taxonomy_ms, classifier_ms.classifier_id == Taxonomy_ms.classifier_id)
...
def get_all_classifiers(
    ...
    with session_factory() as session:
        stmt = (
            select(classifier_ms, Taxonomy_ms)
            .join(Taxonomy_ms, classifier_ms.classifier_id == Taxonomy_ms.classifier_id)
            .order_by(classifier_ms.classifier_name.asc(), Taxonomy_ms.order.asc())
        )
