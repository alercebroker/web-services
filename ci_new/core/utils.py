...
PackageArgs = TypedDict(
    "PackageArgs", {"build-arg": list[str], "value": list[str], "package-dir": str}
)

ChartArgs = TypedDict("ChartArgs", {"chart": str, "values": str, "chart-folder": str})
...
async def git_push(dry_run: bool):
    ...
    async with dagger.Connection(config) as client:
        ...
        container = (
            ...
            .with_exec(["git", "config", "--global", "user.name", '"@alerceadmin"'])
            .with_exec(
                [
                    "git",
                    "config",
...
async def build(
    ...
    bargs = [dagger.BuildArg(arg, value) for arg, value in build_args]
...
async def publish_lib(client: dagger.Client, package_dir, dry_run: bool):
    ...
    source = (
        ...
        .with_workdir(f"/build/{package_dir}")
        .with_exec(["poetry", "config", "pypi-token.pypi", os.environ["PYPI_TOKEN"]])
        .with_exec(command)
    )
...
async def get_tags(client: dagger.Client, package_dir: str) -> list:
    ...
    runner = source.with_workdir(f"/build/{package_dir}").with_exec(
...
def update_chart(container: dagger.Container, chart_name, app_version, dry_run: bool):
    script = [
        "poetry",
        "run",
        "python",
