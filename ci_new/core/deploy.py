...
async def deploy_package(
    packages: dict[str, ChartArgs], cluster_name: str, cluster_alias: str, dry_run: bool
):
        ...
        async with anyio.create_task_group() as tg:
            for package in packages:
                chart_content = yaml.load(
                    open(f"../charts/{packages[package]['chart-folder']}/Chart.yaml"),
                    Loader=SafeLoader,
                )
...
def deploy(packages: dict[str, ChartArgs], stage: Stage, dry_run: bool):
    match stage:
        case Stage.staging:
